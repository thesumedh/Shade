...
async function runTest() {
  ...
  const dockerEnv = new DockerComposeEnvironment(path.resolve(currentDir, '..'), 'standalone.yml')
    .withWaitStrategy('shade-proof-server', Wait.forLogMessage('Actix runtime found; starting in Actix runtime', 1))
    .withWaitStrategy('shade-indexer', Wait.forLogMessage(/starting indexing/, 1));
